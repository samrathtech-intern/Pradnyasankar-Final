...
export function ProductShowcase() {
  ...
  useEffect(() => {
    ...
    const rangeHandler = (e: Event) => {
      const custom = e as CustomEvent<string>;
      setTab(custom.detail as (typeof tabs)[number]);
      setGoal(null);
      setActiveIndex(0);
    };
    window.addEventListener("pradnya:set-focus", handler);
    window.addEventListener("pradnya:set-range", rangeHandler);
    return () => {
      window.removeEventListener("pradnya:set-focus", handler);
      window.removeEventListener("pradnya:set-range", rangeHandler);
    };
  }, []);
  ...
  return (
    <section id="featured" className="relative overflow-hidden bg-white py-8 sm:py-10">
      ...
      <div className="relative container-page">
        ...
        >
          <div>
            ...
            <motion.p
              variants={headerChild}
              className="mt-3 max-w-2xl text-[13px] leading-[1.7] text-[#716A78]"
            >
        ...
        {/* ── Two-column grid ── */}
        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] xl:items-start">
            ...
            <div className="relative">
              ...
              >
                <AnimatePresence mode="wait">
                  ...
                  >
                    {/* ── Image stage ── */}
                    <div className="relative min-h-[260px] overflow-hidden bg-gradient-to-br from-[#F4EEFF] via-[#FAF6FF] to-[#FFF8F0] sm:min-h-[300px]">
                    ...
                    <div className="border-t border-[#EDE6F4] bg-white/85 p-4 backdrop-blur-sm sm:p-5">
                      {/* Badges row */}
                      ...
                      {/* Product name */}
                      <h3 className="mt-2 text-[clamp(20px,2.4vw,32px)] font-extrabold leading-[1.02] tracking-[-.05em] text-[#2E0569]">
                        {activeProduct.name}
                      </h3>

                      {/* Descriptor */}
                      <p className="mt-1.5 text-[12px] leading-[1.6] text-[#716A78]">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-3">
                        ...
                        <motion.button
                          ...
                          className={`group inline-flex min-h-[44px] items-center gap-2.5 rounded-full px-6 text-[10px] font-extrabold uppercase tracking-[.12em] transition-all duration-300 ${
                            inBag
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.96 }}
                          onClick={() => setQuickView(activeProduct)}
                          className="inline-flex min-h-[44px] items-center gap-2 rounded-full border border-[#DCCFE8] bg-white px-5 text-[10px] font-extrabold uppercase tracking-[.1em] text-[#2E0569] transition duration-300 hover:border-[#8C52FF] hover:text-[#8C52FF] hover:shadow-[0_8px_24px_rgba(140,82,255,.12)]"
                        >
          ...
          <Reveal delay={0.08}>
            <div className="flex flex-col gap-1.5">
                ...
                {selectors.map((product, index) => {
                  ...
                  return (
                    <motion.article
                      ...
                      whileHover={reduce ? undefined : { y: -3, transition: { duration: 0.28, ease: [0.22, 1, 0.36, 1] } }}
                      className={`group relative flex flex-col overflow-hidden rounded-[20px] bg-white border transition-shadow duration-300 ${
                        isSelected
                          ? "border-[#8C52FF] shadow-[0_0_0_2px_#8C52FF,0_8px_24px_rgba(140,82,255,.18)]"
                          : "border-[#EDE6F4] shadow-[0_2px_12px_rgba(46,5,105,.07)] hover:shadow-[0_8px_28px_rgba(46,5,105,.14)]"
                      }`}
                    >
                      {/* status badge */}
                      ...
                      <button
                        ...
                        className="relative w-full aspect-[3/2] flex-shrink-0 cursor-zoom-in bg-[#FAF6FF]"
                      >
                        <Image
                          src={product.image}
                          alt={product.name}
                      ...
                      <button
                        ...
                        className="flex flex-col border-t border-[#EDE6F4] px-3 pt-1.5 pb-2 text-left"
                      >
                        <span className="text-[8px] font-extrabold uppercase tracking-[.13em] text-[#8C52FF] leading-none">
                          {product.range}
                        </span>
                        <strong className="mt-0.5 text-[12px] font-extrabold leading-tight tracking-[-.02em] text-[#2E0569]">
                          {product.name}
                        </strong>
                        <span className="mt-0.5 inline-flex items-center gap-1 text-[9px] font-extrabold uppercase tracking-[.1em] text-[#716A78] transition-all duration-300 group-hover:text-[#8C52FF] group-hover:gap-1.5">
                          Show product <ArrowRight size={9} />
                        </span>
                      </button>
                    </motion.article>
                  );
              ...
              {/* Complete Collection card — full width below grid */}
              {filtered.length > selectors.length && (
                <motion.a
                  href="#featured"
                  whileHover={reduce ? undefined : { y: -2 }}
                  className="group flex items-center justify-between gap-4 rounded-[20px] border border-[#EDE6F4] bg-white px-5 py-3 text-left transition duration-300 hover:border-[#8C52FF] hover:shadow-[0_8px_24px_rgba(140,82,255,.10)]"
                >
                  <div>
                    <p className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[#8C52FF]">
                      Complete collection
                    </p>
                    <p className="mt-0.5 text-[15px] font-extrabold text-[#2E0569]">
                  ...
                  <motion.span
                    ...
                    transition={{ duration: 1.6, repeat: Infinity, ease: "easeInOut" }}
                    className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-gradient-to-br from-[#8C52FF] to-[#2E0569] text-white shadow-[0_6px_16px_rgba(140,82,255,.28)]"
                  >
                    <ArrowRight size={16} />
                  </motion.span>
                </motion.a>
