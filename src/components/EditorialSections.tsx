      ...
      <div className="relative container-page">
        ...
        <div className="mt-5 grid items-stretch gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <Reveal className="h-full">
            <Link href={`/knowledge/${articles[0].slug}`} className="group flex h-full flex-col overflow-hidden rounded-[36px] border border-[#E9E3EE] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(46,5,105,.12)]">
              <div className="relative min-h-[240px] flex-1 overflow-hidden sm:min-h-0"><Image src={articles[0].image} alt={articles[0].title} fill className="object-cover transition duration-700 group-hover:scale-[1.035]" /></div>
              <div className="shrink-0 bg-[#2E0569] p-5 text-white sm:p-6"><span className="rounded-full bg-white/10 px-4 py-2 text-[9px] font-extrabold uppercase tracking-[.14em]">{articles[0].category}</span><h3 className="mt-3 max-w-2xl text-[clamp(22px,2.4vw,34px)] font-extrabold leading-[1.04] tracking-[-.05em]">{articles[0].title}</h3><p className="mt-2 max-w-xl text-[13px] leading-[1.75] text-white/[.72]">{articles[0].copy}</p><span className="mt-4 inline-flex items-center gap-2 text-[9px] font-extrabold uppercase tracking-[.13em] text-[#FFCF85]">Read article <ArrowRight size={15} /></span></div>
            </Link>
          </Reveal>
